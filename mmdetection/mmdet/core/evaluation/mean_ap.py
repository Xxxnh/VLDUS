...
def eval_map(det_results,
    ...
    if scale_ranges is not None:
        ...
        if dataset_name == 'coco':
            split_ind = 65
        elif dataset_name == 'vd' or dataset_name == 'voc' or dataset_name == 'dior':
            split_ind = 16
        else: 
            split_ind = 16
        mean_ap_seen = np.array(aps[split_ind:]).mean().item() if aps else 0.0
        mean_ap_unseen = np.array(aps[:split_ind]).mean().item() if aps else 0.0
        mean_ap = np.array(aps).mean().item() if aps else 0.0
        # mean_recall = np.array(recalls).mean().item() if recalls else 0.0
    if print_summary:
        record = []
        record = print_map_summary(aps, eval_results, dataset, dataset_name, area_ranges, recalls=recalls, split=split)

    # return mean_ap, eval_results
    return record[0], record[1], mean_ap, eval_results
        ...
        for ind in range(len(indexes)):
            ...
            for j in classes_inds:
                if num_gts[i, j] > 0:
                    ...
                    curr_precision.append(precisions[i, j])
            ...
            mean_precision = np.array(curr_precision).mean().item() if len(curr_recalls) > 0 else 0.0
            table_data.append(['mean', '', '', '{:.3f}'.format(mean_recall), '{:.3f}'.format(mean_precision), '{:.3f}'.format(mean_ap)])
            means_table.append(['mean', '', '', '{:.3f}'.format(mean_recall), '{:.3f}'.format(mean_precision), '{:.3f}'.format(mean_ap)])
            table = AsciiTable(table_data)
            table.inner_footing_row_border = True
            
            record.append(mean_ap)
            print(table.table)
