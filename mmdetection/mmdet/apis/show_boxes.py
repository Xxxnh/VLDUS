...
#         print(label_text,unseen_labels)
...
def imshow_det_bboxes(img,
                      ...
                      class_names=None,
                      score_thr=0,
                      bbox_color=(34, 139, 34),  # Forest Green
                      text_color=(255, 255, 255),  # White
                      thickness=2,
                      font_scale=0.8,  # Adjust font scale to a smaller value
                      show=True,
                      win_name='',
                      wait_time=0,
                      out_file=None,
    ...
    for bbox, label in zip(bboxes, labels):
        ...
        if label_text in unseen_labels:
            current_bbox_color = (225, 0, 0)  # Dark Red
        else:
            current_bbox_color = (34, 139, 34)  # Forest Green

        cv2.rectangle(img, left_top, right_bottom, current_bbox_color, thickness=thickness)

        label_text = label_text.lower().replace('_', ' ')
        txt_thickness = 2  # Keep text thickness for bold effect
        font = cv2.FONT_HERSHEY_SIMPLEX
        (text_width, text_height) = cv2.getTextSize(label_text, font, fontScale=font_scale, thickness=txt_thickness)[0]
        text_offset_x = bbox_int[0]
        text_offset_y = bbox_int[1] - 2
        box_coords = ((text_offset_x, text_offset_y), (text_offset_x + text_width, text_offset_y - text_height))
        cv2.rectangle(img, box_coords[0], box_coords[1], current_bbox_color, cv2.FILLED)
        cv2.putText(img, label_text, (bbox_int[0], bbox_int[1] - 2), font, font_scale, text_color, thickness=txt_thickness)
