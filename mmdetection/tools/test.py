...
def eval(result_file, dataset, iou_thr=0.5, dataset_name='voc', split='16_4'):
    ...
    mean_ap_seen, mean_ap_unseen, mean_ap, eval_results = eval_map(
        det_results,
        gt_bboxes,
        gt_labels,
        gt_ignore=gt_ignore,
    ...
    return mean_ap_seen, mean_ap_unseen, mean_ap, eval_results
...
def main():
    ...
    if cfg.test_cfg.rcnn.gzsd and cfg.test_cfg.rcnn.dataset_name=='voc':
        cfg.data.test.classes_to_load = 'all'
    dataset = build_dataset(cfg.data.test)
    # print(len(dataset))
    if cfg.test_cfg.rcnn.gzsd and cfg.test_cfg.rcnn.dataset_name=='coco':
        dataset.cat_to_load = dataset.cat_ids
    ...
    if args.out and rank == 0:
        ...
        mean_ap_seen, mean_ap_unseen, mean_ap, _ = eval(args.out, dataset, dataset_name=dataset_name, split=cfg.data.test.split)
        hm = 2*mean_ap_seen*mean_ap_unseen/(mean_ap_seen+mean_ap_unseen)
        f = open('results.txt', 'a')
        # f.writelines(f"{mean_ap} -------- {args.syn_weights} \n")    # import pdb; pdb.set_trace()
        f.writelines(f"{mean_ap_seen}  |||  {mean_ap_unseen} ||| {hm}-------- {args.syn_weights} \n")    # import pdb; pdb.set_trace()
        f.close()
